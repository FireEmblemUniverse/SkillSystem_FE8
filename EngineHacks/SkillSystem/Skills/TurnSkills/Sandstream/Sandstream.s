.thumb
.org 0x0
.equ SandstreamID, SkillTester+4
...
@check to see if the weather has already been set to sunny, or snowy or rainy
...
beq End                         @if they're equal, then the weather is already snowy and we can't override it
cmp r1,#4                       @compare the weather byte to the rain weather byte
beq End                         @if they're equal, then the weather is already raining and we can't override it
...
look_for_unit__asdfgh:			@Name mangling for copy-pasta-bility
    ...
    @change weather to sand storm
    Sandstream:
    ldr  r0, =ChapterData
    mov  r1, #0x5               @weather byte - sand storm
