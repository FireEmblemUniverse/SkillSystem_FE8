...
@ arm/soar_wram.arm.c:100: 		ObjInsertSafe(8, 0x10, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3), 2, 0xe))); //draw in the top corner if we're there
	mov	r2, #16	@,
@ arm/soar_wram.arm.c:100: 		ObjInsertSafe(8, 0x10, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3), 2, 0xe))); //draw in the top corner if we're there
	sub	r5, r3, #1	@ tmp269, loc,
	lsl	r5, r5, #19	@ tmp272, tmp269,
	lsr	r5, r5, r2	@ _67, tmp272,
	add	r3, r5, #576	@ tmp274, _67,
@ arm/soar_wram.arm.c:100: 		ObjInsertSafe(8, 0x10, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3), 2, 0xe))); //draw in the top corner if we're there
	orr	r3, r3, #59392	@ tmp278, tmp274,
@ arm/soar_wram.arm.c:101: 		ObjInsertSafe(8, 0x30, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3)+4, 2, 0xe))); //draw in the top corner if we're there
	add	r5, r5, #580	@ tmp283, _67,
@ arm/soar_wram.arm.c:100: 		ObjInsertSafe(8, 0x10, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3), 2, 0xe))); //draw in the top corner if we're there
	lsl	r3, r3, r2	@ tmp280, tmp278,
@ arm/soar_wram.arm.c:101: 		ObjInsertSafe(8, 0x30, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3)+4, 2, 0xe))); //draw in the top corner if we're there
	orr	r5, r5, #59392	@ tmp287, tmp283,
@ arm/soar_wram.arm.c:100: 		ObjInsertSafe(8, 0x10, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3), 2, 0xe))); //draw in the top corner if we're there
	lsr	r3, r3, r2	@ tmp279, tmp280,
@ arm/soar_wram.arm.c:101: 		ObjInsertSafe(8, 0x30, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3)+4, 2, 0xe))); //draw in the top corner if we're there
	lsl	r5, r5, #16	@ tmp289, tmp287,
@ arm/soar_wram.arm.c:100: 		ObjInsertSafe(8, 0x10, 0x10, &gObj_32x8, (OAM_ATTR2(LocationBaseTID+((loc-1)<<3), 2, 0xe))); //draw in the top corner if we're there
	mov	r1, r2	@,
	str	r3, [sp]	@ tmp279,
	mov	r0, #8	@,
...
@ arm/soar_wram.arm.c:13: 		iwram_Render_arm(CurrentProc);
	mov	r0, r4	@, CurrentProc
	ldr	r3, .L45+68	@ tmp292,
	mov	lr, pc
	bx	r3		@ tmp292
@ arm/soar_wram.arm.c:15: 		FPS_COUNTER += 1;
	ldr	r2, .L45+60	@ tmp293,
	ldr	r3, [r2, #4088]	@ MEM[(int *)33816568B], MEM[(int *)33816568B]
	add	r3, r3, #1	@ tmp295, MEM[(int *)33816568B],
	str	r3, [r2, #4088]	@ tmp295, MEM[(int *)33816568B]
@ arm/soar_wram.arm.c:17: };
	add	sp, sp, #12	@,,
	@ sp needed	@
	pop	{r4, r5, r6, r7, lr}	@
...
.L19:
@ arm/soar_wram.arm.c:72: 		switch(CurrentProc->sPlayerYaw){
	mov	r1, #64	@ prephitmp_137,
	b	.L11		@
.L18:
	mov	r1, #128	@ prephitmp_137,
	b	.L11		@
.L44:
@ arm/soar_wram.arm.c:93: 		if (CurrentProc->ShowMap) ObjInsertSafe(8, (176 + (posX>>4)), (posY-MAP_YOFS)>>4, &gObj_8x8, OAM_ATTR2(CursorBaseTID+CurrentProc->sPlayerYaw, 2, 0xD)); //draw cursor on minimap
	ldr	r3, [r4, #60]	@ CurrentProc_6(D)->sPlayerYaw, CurrentProc_6(D)->sPlayerYaw
	add	r3, r3, #704	@ tmp253, CurrentProc_6(D)->sPlayerYaw,
@ arm/soar_wram.arm.c:93: 		if (CurrentProc->ShowMap) ObjInsertSafe(8, (176 + (posX>>4)), (posY-MAP_YOFS)>>4, &gObj_8x8, OAM_ATTR2(CursorBaseTID+CurrentProc->sPlayerYaw, 2, 0xD)); //draw cursor on minimap
	asr	r1, r7, #4	@ tmp246, posX,
@ arm/soar_wram.arm.c:93: 		if (CurrentProc->ShowMap) ObjInsertSafe(8, (176 + (posX>>4)), (posY-MAP_YOFS)>>4, &gObj_8x8, OAM_ATTR2(CursorBaseTID+CurrentProc->sPlayerYaw, 2, 0xD)); //draw cursor on minimap
	orr	r3, r3, #55296	@ tmp257, tmp253,
@ arm/soar_wram.arm.c:93: 		if (CurrentProc->ShowMap) ObjInsertSafe(8, (176 + (posX>>4)), (posY-MAP_YOFS)>>4, &gObj_8x8, OAM_ATTR2(CursorBaseTID+CurrentProc->sPlayerYaw, 2, 0xD)); //draw cursor on minimap
	asr	r2, r5, #4	@ tmp243, posY,
@ arm/soar_wram.arm.c:93: 		if (CurrentProc->ShowMap) ObjInsertSafe(8, (176 + (posX>>4)), (posY-MAP_YOFS)>>4, &gObj_8x8, OAM_ATTR2(CursorBaseTID+CurrentProc->sPlayerYaw, 2, 0xD)); //draw cursor on minimap
	lsl	r3, r3, #16	@ tmp258, tmp257,
	add	r1, r1, #176	@ tmp248, tmp246,
	lsr	r3, r3, #16	@ tmp258, tmp258,
	lsl	r2, r2, #16	@, tmp243,
	lsl	r1, r1, #16	@, tmp248,
	str	r3, [sp]	@ tmp258,
	mov	r0, #8	@,
