...
cmp r0, #0
bne ReturnPoint
@add CompletionFlag check here to prevent it from spawning I think
...
@push {r5, lr}
push {lr}
@ldrb r0,[r5,#3] @Completion Flag
mov r0, #0xb0 @unused permanent event id
ldr r3,=0x8083da9 //vanilla CheckFlag routine
bl goto_r3
@pop {r5}
pop {pc}
...
@if its completion flag is on, not usable 
@bl FlagCheck
