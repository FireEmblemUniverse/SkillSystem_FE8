...
const u16 KonamiCodeSequence[] = {DPAD_UP,   DPAD_UP,    DPAD_DOWN, DPAD_DOWN,
                                  DPAD_LEFT, DPAD_RIGHT, DPAD_LEFT, DPAD_RIGHT,
                                  B_BUTTON,  A_BUTTON,   0,         0};
...
void DisplayTilesetTile(DebuggerProc *proc, u16 *bg, int xTileMap, int yTileMap,
                        int xBmMap, int yBmMap);
...
const struct ProcCmd DebuggerProcCmd[] = {
    ...
    PROC_CALL_2(ClearActiveUnitStuff), // in case we didn't refresh units before
                                       // restarting
...
void CpOrderBerserkInit(ProcPtr proc) {
  ...
  for (i = 0; i < factionUnitCountLut[faction >> 6]; ++i) {
    ...
    if (unit->state &
        (US_HIDDEN | US_UNSELECTABLE | US_DEAD | US_RESCUED | US_HAS_MOVED_AI))
      continue;
...
void DebuggerListInit(DebuggerProc *parent) {
  ...
  struct PrepItemListProc *proc =
      Proc_StartBlocking(DebuggerProcScr_PrepItemListScreen, parent);
  proc->unit = gActiveUnit;
}
void DebuggerListIdle(DebuggerProc *proc) { return; }
...
static const int DigitHexTable[] = {0x1,       0x10,       0x100,
                                    0x1000,    0x10000,    0x100000,
                                    0x1000000, 0x10000000, 0x7fffffff};
...
void RedrawUnitStatsMenu(DebuggerProc *proc) {
  ...
  for (int i = 0; i < NumberOfOptions; ++i) {
    PutText(&th[i],
            gBG0TilemapBuffer + TILEMAP_INDEX(x, (Y_HAND - 1) + (i * 2)));
  }

  for (int i = 0; i < NumberOfOptions; ++i) {
    PutNumber(gBG0TilemapBuffer + TILEMAP_INDEX(START_X, Y_HAND - 1 + (i * 2)),
...
static void DisplayVertUiHand(int x, int y) {
  ...
  y += (sHandVOffsetLookup[Mod(GetGameClock(),
                               ARRAY_COUNT(sHandVOffsetLookup))] -
        14);
...
void EditStatsIdle(DebuggerProc *proc) {
  ...
  if ((keys & START_BUTTON) ||
      (keys & A_BUTTON)) { // press A or Start to update stats and continue
    SaveStats(proc);
    Proc_Goto(proc, RestartLabel);
    m4aSongNumStart(0x6B);
  };
  if (proc->editing) {
    ...
    DisplayUiHand(CursorLocationTable[0].x - ((StatWidth + 2) * 8),
                  (Y_HAND - 1 + (proc->id * 2)) * 8);
    if (keys & DPAD_RIGHT) {
      proc->digit = 1;
      proc->editing = true;
...
void EditWExpInit(DebuggerProc *proc) {
  ...
  DrawUiFrame(BG_GetMapBuffer(2),            // back BG
              x, y, w, h, TILEREF(0, 0), 0); // style as 0 ?
...
void DebuggerDisplayWeaponExp(int num, int x, int y, int wtype, int wexp) {
  ...
  // Display rank letter
  PutSpecialChar(gBG0TilemapBuffer + TILEMAP_INDEX(x + 4, y), color,
...
void EditWExpIdle(DebuggerProc *proc) {
  ...
  if ((keys & START_BUTTON) ||
      (keys & A_BUTTON)) { // press A or Start to update WExp and continue
    SaveWExp(proc);
    ClearTilesetRow(proc);
    Proc_Goto(proc, RestartLabel);
    m4aSongNumStart(0x6B);
  ...
  if (proc->editing) {
    ...
    DisplayUiHand(CursorLocationTable[0].x - ((WExpWidth + 2) * 8),
                  (Y_HAND + (proc->id * 2)) * 8);
    if (keys & DPAD_RIGHT) {
      proc->digit = 1;
      proc->editing = true;
...
void EditSupportsInit(DebuggerProc *proc) {
  ...
  if (unit->pCharacterData->pSupportData) {
    ...
    for (int i = 0; i < SupportOptions; ++i) {
      ...
      if (uid) {
        Text_DrawString(
            &th[i], GetStringFromIndexSafe(GetCharacterData(uid)->nameTextId));
      }
    }
  }
...
void EditSupportsIdle(DebuggerProc *proc) {
  ...
  if ((keys & START_BUTTON) ||
      (keys & A_BUTTON)) { // press A or Start to update Supports and continue
    SaveSupports(proc);
    Proc_Goto(proc, RestartLabel);
    m4aSongNumStart(0x6B);
  };
  if (proc->editing) {
    ...
    DisplayUiHand(CursorLocationTable[0].x - ((SupportWidth + 2) * 8),
                  (Y_HAND + (proc->id * 2)) * 8);
    if (keys & DPAD_RIGHT) {
      proc->digit = 1;
      proc->editing = true;
...
static const char states[32][16] = {
    "Acting",       "Acted",       "Dead",        "Undeployed",  "Rescuing",
    "Rescued",      "Cantoed",     "Under roof",  "Spotted",     "Concealed",
    "AI decided",   "In ballista", "Drop item",   "Afa's drops", "Solo anim1",
    "Solo anim2",   "Escaped",     "Arena 1",     "Arena 2",     "Super arena",
    "Unk 25",       "Benched",     "Scene unit",  "Portrait+1",  "Shake",
    "Can't deploy", "Departed",    "4th palette", "Unk 35",      "Unk 36",
    "Capture",      "Unk 38",
};
...
void StateIdle(DebuggerProc *proc) {
  u16 keys = sKeyStatusBuffer.repeatedKeys;
  if ((keys & START_BUTTON) ||
      (keys & B_BUTTON)) { // press B or Start to update state and continue
...
void RedrawItemMenu(DebuggerProc *proc) {
  ...
  for (int i = 0; i < NumberOfItems; ++i) { // item id
    ...
    PutNumberHex(gBG0TilemapBuffer + TILEMAP_INDEX(START_X, Y_HAND + (i * 2)),
                 TEXT_COLOR_SYSTEM_GOLD, n);
  }
  ...
  for (int i = 0; i < NumberOfItems; ++i) { // uses
    ...
    PutNumber(gBG0TilemapBuffer + TILEMAP_INDEX(START_X + 3, Y_HAND + (i * 2)),
              TEXT_COLOR_SYSTEM_GOLD, n);
  }
  ...
  for (int i = 0; i < NumberOfItems; ++i) {
    ...
    if (icon >= 0) {
      if (proc->tmp[i]) {
        DrawIcon(TILEMAP_LOCATED(gBG0TilemapBuffer, x - 2, Y_HAND + (i * 2)),
                 icon, 0x4000);
      }
    }
  }
...
void EditItemsIdle(DebuggerProc *proc) {
  ...
  if (proc->editing) {
    if (proc->editing == 1) {
      ...
      DisplayVertUiHand(CursorLocationTable[proc->digit].x + (3 * 8),
                        (Y_HAND + (proc->id * 2)) * 8);
    ...
    DisplayUiHand(CursorLocationTable[0].x - ((ItemNameWidth + 4) * 8),
                  (Y_HAND + (proc->id * 2)) * 8);
    if (keys & DPAD_RIGHT) {
      proc->digit = 1;
      proc->editing = true;
...
static const char weatherStates[][16] = {
    "Clear", "Snowy",   "Blizzard",  "Night",
    "Rainy", "Volcano", "Sandstorm", "Cloudy",
};
...
int GetChStateMax(int id) { return chStateMax[id]; }
int GetChStateMin(int id) { return chStateMin[id]; }
...
void RedrawChStateMenu(DebuggerProc *proc) {
  ...
  Text_DrawString(&th[i],
                  GetStringFromIndexSafe(
                      GetROMChapterStruct(proc->tmp[3])->chapTitleTextId));
  i++;
  ClearText(&th[i]);
  Text_DrawString(&th[i],
                  GetStringFromIndexSafe(
                      GetROMChapterStruct(proc->tmp[4])->chapTitleTextId));
  i++;
  ...
  PutNumber(gBG0TilemapBuffer + TILEMAP_INDEX(START_X + 4, Y_HAND),
            TEXT_COLOR_SYSTEM_GOLD, proc->gold);
  ...
  for (i = 1; i < NumberOfChState; ++i) {
    ...
    if (hexOrDec) {
      PutNumberHex(gBG0TilemapBuffer +
                       TILEMAP_INDEX(START_X + 4, Y_HAND + (i * 2)),
                   col, proc->tmp[i]);
    } else {
      PutNumber(gBG0TilemapBuffer +
                    TILEMAP_INDEX(START_X + 4, Y_HAND + (i * 2)),
                col, proc->tmp[i]);
    }
  }
...
void ChStateIdle(DebuggerProc *proc) {
  ...
  if ((keys & START_BUTTON) ||
      (keys & A_BUTTON)) { // press A or Start to update ch state and continue
    SaveChState(proc);
    if (proc->id != 6) {
      Proc_Goto(proc, RestartLabel);
      m4aSongNumStart(0x6B);
  ...
  if (proc->editing && (type >= 0)) {
    DisplayVertUiHand(CursorLocationTable[proc->digit].x + 32,
                      (Y_HAND + (id * 2)) * 8);
    ...
    DisplayUiHand(CursorLocationTable[0].x - ((ChStateNameWidth + 5) * 8),
                  (Y_HAND + (id * 2)) * 8);
    if (keys & DPAD_RIGHT) {
...
void RedrawMiscMenu(DebuggerProc *proc) {
  ...
  Text_DrawString(&th[i], GetStringFromIndexSafe(
                              GetCharacterData(proc->tmp[0])->nameTextId));
  i++;
  Text_DrawString(
      &th[i], GetStringFromIndexSafe(GetClassData(proc->tmp[1])->nameTextId));
  i++;
  ...
  if (!proc->tmp[6]) {
    ...
    Text_DrawString(
        &th[i], GetStringFromIndexSafe(sStatusNameTextIdLookup[proc->tmp[6]]));
    i++;
  }
...
void EditMiscIdle(DebuggerProc *proc) {
  ...
  if (proc->editing) {
    ...
    DisplayUiHand(CursorLocationTable[0].x - ((MiscNameWidth + 2) * 8),
                  (Y_HAND + (proc->id * 2)) * 8);
    if (keys & DPAD_RIGHT) {
      proc->digit = 1;
      proc->editing = true;
...
static void InitUnitDef(struct UnitDefinition *uDef, struct Unit *unit,
...
const u8 BasicWeaponsByType[] = {
    ITEM_SWORD_IRON,      ITEM_LANCE_IRON, ITEM_AXE_IRON,
    ITEM_BOW_IRON,        ITEM_STAFF_HEAL, ITEM_ANIMA_FIRE,
    ITEM_LIGHT_LIGHTNING, ITEM_DARK_FLUX,  ITEM_MONSTER_ROTTENCLW,
    ITEM_LOCKPICK,        ITEM_ELIXIR,     ITEM_VULNERARY};
...
void FindNearestTile(struct Unit *unit) {
  ...
  GenerateExtendedMovementMap(unit->xPos, unit->yPos,
...
void LoadAllUnits(int type, int uid) {
  ...
  if (type != ExistingUnits) {
    for (; i < end; ++i) {
      ...
      charIDsToIgnore[tmp >> 5] |=
          1 << (tmp & 0x1F); // make 8 bitfields of unitIDs to ignore
...
int GetLoadMax(int val) { return 0xFF; }
int GetLoadMin(int val) { return 0x1; }
...
void RedrawLoadMenu(DebuggerProc *proc) {
  ...
  i++;
  Text_DrawString(&th[i], GetStringFromIndexSafe(
                              GetCharacterData(proc->tmp[1])->nameTextId));
  i++;
  Text_DrawString(&th[i], GetStringFromIndexSafe(
                              GetCharacterData(proc->tmp[2])->nameTextId));
  i++;
...
void LoadUnitsIdle(DebuggerProc *proc) {
  ...
  if (proc->editing) {
    ...
    DisplayUiHand(CursorLocationTable[0].x - ((LoadNameWidth + 2) * 8),
                  (Y_HAND + (proc->id * 2)) * 8);
    if (keys & DPAD_RIGHT) {
      proc->digit = 1;
      proc->editing = true;
  ...
  for (iy = (10 - 1); iy >= 9; --iy) // 9 so only bottom row
    for (ix = (15 - 1); ix >= 0; --ix)
      DisplayTilesetTile(proc, gBG2TilemapBuffer, ix, iy,
                         (short)gBmSt.mapRenderOrigin.x + ix,
...
void EditMapIdle(DebuggerProc *proc) {
  ...
  PutMapCursor(gBmSt.playerCursorDisplay.x, gBmSt.playerCursorDisplay.y,
               IsUnitSpriteHoverEnabledAt(x, y) ? 3 : 0);
}
...
void PickupUnitIdle(DebuggerProc *proc) {
  ...
  PutMapCursor(
      ...
      IsUnitSpriteHoverEnabledAt(gBmSt.playerCursor.x, gBmSt.playerCursor.y)
          ? 3
          : 0);
}
...
int ClearActiveUnitStuff(DebuggerProc *proc) {
  ...
  s8 cameraReturn = EnsureCameraOntoPositionIfValid(
      proc, gActiveUnitMoveOrigin.x, gActiveUnitMoveOrigin.y);
  cameraReturn ^= 1;
...
int PlayerPhase_PrepareActionBasic(DebuggerProc *proc) {
  ...
  cameraReturn = EnsureCameraOntoPositionIfValid(
      proc, GetUnit(gActionData.subjectIndex)->xPos,
      GetUnit(gActionData.subjectIndex)->yPos);
  cameraReturn ^= 1;
...
void ComputeBattleUnitEffectiveStats(struct BattleUnit *attacker,
...
u8 AiControlRemainingUnitsNow(struct MenuProc *menu,
...
void SetupUnitFunc(void) {
  ...
  gBattleActor.weapon = gBattleTarget.weapon =
      GetUnitEquippedWeapon(GetUnit(gActionData.subjectIndex));
...
u8 CanActiveUnitPromote(void) {
  ...
  if ((!promoTable[classNumber * 2]) &&
      (!promoTable[(classNumber * 2) + 1])) { // gPromoJidLut[classNumber][0];
    return 2;                                 // greyed out
  }
...
int DebuggerMenuItemDraw(struct MenuProc *menu, struct MenuItemProc *menuItem) {
  ...
  Text_DrawString(&menuItem->text,
                  GetDebuggerMenuText(procIdler, menuItem->itemNumber));
  PutText(&menuItem->text, BG_GetMapBuffer(menu->frontBg) +
...
int AiControlRemainingUnitsDrawText(struct MenuProc *menu,
                                    struct MenuItemProc *menuItem) {
  if (menuItem->availability == MENU_DISABLED) {
    Text_SetColor(&menuItem->text, 1);
  }
...
void PageMenuItemDrawSprites(struct MenuProc *menu) {
  ...
  PutSprite(0, x, y, gObject_8x8,
            TILEREF(chr, 0) + OAM2_LAYER(0) + proc->page + 1);
  x += 8;
  PutSprite(0, x, y, gObject_8x8, TILEREF(chr, 0) + OAM2_LAYER(0));
  x += 8;
  PutSprite(0, x, y, gObject_8x8,
            TILEREF(chr, 0) + OAM2_LAYER(0) + NumberOfPages);
  x += 8;
}
...
struct Unit *GetPrevUnit(int deployId, int allegiance) {
  ...
  for (int i = deployId - 1; i >= allegiance;
       --i) // should loop back to itself I guess
  {
    unit = GetUnit(i);
    if (UNIT_IS_VALID(unit)) {
      return unit;
  ...
  for (int i = ((allegiance & 0xC0) + 0x3F); i > deployId;
       --i) // should loop back to itself I guess
  {
    unit = GetUnit(i);
    if (UNIT_IS_VALID(unit)) {
...
u8 MenuCancelSelectResumePlayerPhase(struct MenuProc *menu,
...
void StartDebuggerProc(
    ProcPtr playerPhaseProc) { // based on PlayerPhase_MainIdle
  if (!ShouldStartDebugger()) {
    return;
  }
  struct Unit *unit =
      GetUnit(gBmMapUnit[gBmSt.playerCursor.y][gBmSt.playerCursor.x]);
  if (!unit) {
    return;
  }
  gActiveUnitMoveOrigin.x = unit->xPos;
...
void RestartDebuggerMenu(DebuggerProc *proc) {
  struct Unit *unit =
      proc->unit; // GetUnit(gBmMapUnit[gBmSt.playerCursor.y][gBmSt.playerCursor.x]);
  if (!unit) {
    Proc_Goto(proc, EndLabel);
    return;
  }
  ...
  switch (proc->page) {
  case 0: {
    menu = StartOrphanMenuAdjusted(
        &gDebuggerMenuDef, gBmSt.cursorTarget.x - gBmSt.camera.x, 1, 0x15);
    break;
  }
  case 1: {
    menu = StartOrphanMenuAdjusted(
        &gDebuggerMenuDefPage2, gBmSt.cursorTarget.x - gBmSt.camera.x, 1, 0x15);
    break;
  }
  case 2: {
    menu = StartOrphanMenuAdjusted(
        &gDebuggerMenuDefPage3, gBmSt.cursorTarget.x - gBmSt.camera.x, 1, 0x15);
    break;
  }
  default:
  }
...
void LoopDebuggerProc(DebuggerProc *proc) { return; }
...
void StartHelpBoxTextInitWithString(int item, int msgId, char *string) {
  struct ProcHelpBoxIntroString *proc =
      Proc_Start(ProcScr_HelpBoxIntroString, PROC_TREE_3);
...
void ApplyHelpBoxContentSizeString(struct HelpBoxProc *proc, int width,
...
u8 DebuggerHelpBox(struct MenuProc *menu, struct MenuItemProc *item) {
  DebuggerProc *procIdler = Proc_Find(DebuggerProcCmdIdler);
  StartHelpBoxString(item->xTile * 8, item->yTile * 8,
                     GetDebuggerMenuDesc(procIdler, item->itemNumber));
  return 0;
}
