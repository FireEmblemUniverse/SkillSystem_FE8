...
.global IsPeacefulInclNoEnemyTrainers
.type IsPeacefulInclNoEnemyTrainers, %function 
IsPeacefulInclNoEnemyTrainers:
push {r4-r5, lr}

ldr r5, =0x401000C
@ check for if peaceful map 

@ check for at least one valid enemy or trainer that hasn't challenged the player
mov r4,#0x7F @ current deployment id
LoopThroughUnits90:
add r4, #1 
cmp r4, #0xC0 
bge IsPeaceful_True90
mov r0,r4
blh GetUnit @ 19430
cmp r0, #0
beq LoopThroughUnits90
ldr r1, [r0]
cmp r1, #0 
beq LoopThroughUnits90
ldr r1, [r0, #0x0C]
tst r1, r5 @ dead/undeployed 
bne LoopThroughUnits90
ldr r1, [r0] @ char pointer 
ldrb r1, [r1, #4] @ char id 
cmp r1, #0xE0 
blt WillTrainerChallengePlayer
cmp r1, #0xEF 
bgt WillTrainerChallengePlayer

ValidEnemy90:
mov r0, #0 
b Exit90 

WillTrainerChallengePlayer:
mov r1, #0x2D 
ldrb r1, [r0, r1] 
cmp r1, #50
beq LoopThroughUnits90
