...
bool Generic_CanUnitBeOnPos(struct Unit *unit, s8 x, s8 y, int x2, int y2) {
  ...
  return Vanilla_CanUnitCrossTerrain(
      unit,
  ...
  if (gBattleStats.config & (BATTLE_CONFIG_REAL | BATTLE_CONFIG_SIMULATE)) {
    if (gBattleStats.range == 1) {
      if (SkillTester(&bunitB->unit, SwarmID_Link)) {
        ...
        if (dmg < 0)
          dmg = 0;
        int addDmg = ((dmg) * (SwarmBonusDamagePercent)) /
                     100; // dmg+(dmg*SwarmBonusDamagePercent/200) for rounding
        // int addDmg =
...
void FlankEffect(struct BattleUnit *bunitA, struct BattleUnit *bunitB) {
  if (gBattleStats.config & (BATTLE_CONFIG_REAL | BATTLE_CONFIG_SIMULATE)) {
    if (gBattleStats.range == 1) {
      if (SkillTester(&bunitB->unit, FlankID_Link) ||
          (!FlankRequiresSkill_Link)) {
        ...
        if ((allyID) && (AreUnitsAllied(deploymentID, allyID))) {
          ...
          int addDmg =
              ((dmg) * (FlankBonusDamagePercent)) /
              100; // dmg+(dmg*FlankBonusDamagePercent/200) for rounding
          bunitB->battleAttack += addDmg;
        }
      }
    }
...
void SilphScopeEffect(struct BattleUnit *bunitA, struct BattleUnit *bunitB) {
  if (gBattleStats.config & (BATTLE_CONFIG_REAL | BATTLE_CONFIG_SIMULATE)) {
    ...
    if (ShouldWeaponHaveStabBonus(bunitA->weaponBefore,
                                  bunitA->unit.pClassData->number)) {
      if (SkillTester(&bunitA->unit, SilphScopeID_Link)) {
        int effectiveness =
            IsItemEffectiveAgainst(bunitA->weaponBefore, &bunitB->unit);
        if (effectiveness) {
          if ((effectiveness != 1) && (effectiveness != 2) &&
              (effectiveness != 7) && (effectiveness != 9)) {
...
void UnpackChapterMapPalette(void) {
  if ((gActiveUnit) && (gActiveUnit->pCharacterData) &&
      (gActiveUnit->pClassData->number == MissingnoID_Link)) {
    ApplyPalettes(
  ...
  ApplyPalettes(ObjTypePalettePLIST[GetROMChapterStruct(gPlaySt.chapterIndex)
                                        ->map.paletteId],
                6, 10); // TODO: palette id constant?
}
...
void MissingnoEffect(struct Unit *actor, struct Unit *target) {
  if (gActionData.unitActionType == UNIT_ACTION_COMBAT) { // attacking only
    if ((actor->pClassData->number == MissingnoID_Link) ||
        (target->pClassData->number == MissingnoID_Link)) {
