...
int RandomizeStatCap(int statCap, struct Unit* unit, int variance) { 
	...
	if (abs(result - statCap) < 11) { result = HashByte_Global(result, max, classID) + min; } // reroll if too similar 
	if (abs(result - statCap) < 6) { result = HashByte_Global(result, max + min, classID); } // reroll if too similar (no min) 
	
	
	
	if (result >= 60) { result = 60 - HashByte_Global(result, min, classID); } 
	return result; 
