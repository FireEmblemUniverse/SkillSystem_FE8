...
#ifndef POKEMBLEM_VERSION
    if (!(unit->supportBits &
          0x80)) // DRUnitByte & DRUnitBitMask - copied from DisplayDR basically
...
#define FOR_EACH_IN_MOVEMENT_RANGE(block)                                      \
  ...
  if (UNIT_CATTRIBUTES(unit) & CA_BALLISTAE) {
    FOR_EACH_IN_MOVEMENT_RANGE({
      ...
      if (item) {
        MapAddInBoundedRange(ix,
                             iy, // not the new one because item range fix does
                                 // something to the function for ballistas idk
...
struct Unit *GetUnitInline(int id) { return gUnitLookup[id & 0xFF]; }
...
int PokemblemGetUnitWeaponReachBits(struct Unit *unit, int itemSlot) {
...
#ifndef POKEMBLEM_VERSION
          if (CanUnitUseWeapon(unit, item)) { // pokemon can always use moves
  ...
  if (itemSlot >= 0)
    return PokemblemGetItemReachBits(unit->items[itemSlot]) |
           result; // use gaiden magic? or not?
...
int PokemblemGetItemReachBits(int item) {
  if (!(GetItemData(ITEM_INDEX(item))->attributes & IA_WEAPON)) {
    return 0;
  ...
  default:
    return result; // return item encoded range if not one of these cases. 0 if
                   // no range
...
inline void PokemblemMapAddInBoundedRange(short x, short y, short minRange,
                                          short maxRange) {
#ifdef USE_ARM
