...
struct KeyStatusBuffer {
	/*01*/ u8 repeatDelay; // initial delay before generating auto-repeat presses
	/*02*/ u8 repeatInterval; // time between auto-repeat presses
	/*03*/ u8 repeatTimer; // (decreased by one each frame, reset to repeatDelay when Presses change and repeatInterval when reaches 0)
	/*04*/ u16 heldKeys; // keys that are currently held down
	/*06*/ u16 repeatedKeys; // auto-repeated keys
	/*08*/ u16 pressedKeys; // keys that went down this frame
	/* 	A:1; B:2; 
		Right:10; Left:20;  Up:40; Down:80;
		R:100; L:200
		*/
	
	/*0A*/ u16 prevKeys; // keys that were held down last frame
	/*0C*/ u16 lastPressKeys; // keys that were held down last time a new key was
...
extern struct KeyStatusBuffer* gpKeyState; // <Mokha> 0x858791C
