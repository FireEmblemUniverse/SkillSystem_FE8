...
int DoesActorHaveCleatsOrBalloonEquipped(void) {
  int result = false;
  result |= DoesUnitHaveItemWithSpecificDurability(gActiveUnit,
                                                   EquippedAirBalloon_Link);
  result |=
      DoesUnitHaveItemWithSpecificDurability(gActiveUnit, EquippedCleats_Link);
  return result;
}

extern int IceTerrainTypeLink;
bool FMU_CheckForIce(struct FMUProc *proc, int x,
                     int y) { // checks tile we're moving to
...
bool FMU_CanUnitBeOnPos(Unit *unit, s8 x, s8 y) {
  ...
  return FMU_CanUnitCrossTerrain(
      unit, gMapTerrain[y][x]); // CanUnitCrossTerrain(unit, gMapTerrain[y][x]);
}
...
u8 GetFreeMovementState(void) { return FreeMoveRam->state; }
...
void pFMU_DoNothing(struct Proc *proc) { return; }
...
void pFMU_OnInit(struct FMUProc *proc) {
  ...
  if (0 == proc->FMUnit)
    proc->FMUnit =
        gUnitArrayBlue; // if no protag unit, select the first player in ram
