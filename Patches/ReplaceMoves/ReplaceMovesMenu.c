...
static u8 *UnitGetMoveList(struct Unit *unit) { return unit->ranks; }
...
static void MoveListCommandDraw(struct MenuProc *menu,
                                struct MenuCommandProc *command);
static int MoveCommandSelect(struct MenuProc *menu,
                             struct MenuCommandProc *command);
...
void UpdateItemInfo(struct MenuProc *menu, struct MenuCommandProc *command,
                    struct ReplaceMoveProc *proc);
...
void DrawItemInfo(struct MenuProc *menu, struct MenuCommandProc *command,
  ...
  u32 width = (Text_GetStringTextWidth(
...
GetStringFromIndex(GetItemDescId(proc->moveReplacement)));
  ...
  */
  ...
  LoadIconPalettes(4);
  DrawIcon(out + TILEMAP_INDEX(new_item_icon_offset, 0),
...
void UpdateItemInfo(struct MenuProc *menu, struct MenuCommandProc *command,
  ...
  */
  ...
  gSpecialUiCharAllocationTable[0] =
      0xFF; // no clue but it made DrawUiNumber work properly

  DrawUiNumber(&gBG0MapBuffer[15][18 + x], TEXT_COLOR_GOLD,
               GetItemWeight(item));
  ...
  DrawUiNumberOrDoubleDashes(&gBG0MapBuffer[17][18 + x], TEXT_COLOR_GOLD,
                             GetItemCrit(item));
...
static void ReplaceMoveMenuEnd(struct MenuProc *menu) { EndFaceById(0); }
