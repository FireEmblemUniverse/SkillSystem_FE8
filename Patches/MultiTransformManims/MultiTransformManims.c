...
void MTM_TrySetSpecialClassSprite(int actorNum) {
    ...
    for (; it->weaponId != UINT8_MAX; it++) {		
        if ((it->weaponId != item) && (it->weaponId)) { // 0 is any wep 
            continue;
        }
		...
		u8 displayClassId = it->displayClassId;
		if (!it->displayClassId) { 
		displayClassId = gBattleTarget.unit.pClassData->number; 
		if (actorNum) { displayClassId = gBattleActor.unit.pClassData->number; } 
		}
        MU_SetSpecialSprite(gManimSt.actor[actorNum].mu, displayClassId, it->palette);
        return;
    }
